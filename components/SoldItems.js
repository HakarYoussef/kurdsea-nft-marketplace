...
import {
  SoldItemsContainer,
  SoldItemsMarketWrapper,
} from '../styles/SoldItems.style';

export const injected = new InjectedConnector({
  supportedChainIds: [1, 3, 4, 5, 42, 1337, 43114],
});

function SoldItems({ SoldFilteredItems }) {
  ...
  useEffect(() => {
    const loadContracts = async () => {
      ...
      if (nftNetworkObject && marketNetworkObject) {
        ...
        setTimeout(() => {
          const marketSoldItems = items.filter((item) => item.isSold);
  ...
  const lastItems = SoldFilteredItems.slice().reverse();
